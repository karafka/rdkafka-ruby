...
describe Rdkafka::RdkafkaError do
  ...
  describe "#is_partition_eof?" do
    it "is false when not partition eof" do
      assert_equal false, described_class.new(933975).is_partition_eof?
    end

    it "is true when partition eof" do
      assert_equal true, described_class.new(-191).is_partition_eof?
    end
  end

  describe "#==" do
    before do
      @error = described_class.new(10, "Error explanation")
    end

    it "equals another error with the same content" do
      assert_equal described_class.new(10, "Error explanation"), @error
    end

    it "does not equal another error with a different error code" do
      refute_equal described_class.new(20, "Error explanation"), @error
    end

    it "does not equal another error with a different message" do
      refute_equal described_class.new(10, "Different error explanation"), @error
    end

    it "does not equal another error with no message" do
      refute_equal described_class.new(10), @error
    end
