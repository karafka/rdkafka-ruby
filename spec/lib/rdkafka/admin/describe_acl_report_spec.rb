...
RSpec.describe Rdkafka::Admin::DescribeAclReport do
  ...
  def build_report
    ...
    Rdkafka::Admin::DescribeAclReport.new(acls: acls_array_ptr, acls_count: 1)
  end

  it "gets matching acl resource type as Rdkafka::Bindings::RD_KAFKA_RESOURCE_TOPIC" do
    expect(build_report.acls[0].matching_acl_resource_type).to eq(Rdkafka::Bindings::RD_KAFKA_RESOURCE_TOPIC)
  end

  it "gets matching acl resource name" do
    expect(build_report.acls[0].matching_acl_resource_name).to eq(resource_name)
  end
  ...
  it "gets matching acl principal as User:anonymous" do
    expect(build_report.acls[0].matching_acl_principal).to eq("User:anonymous")
  end

  it "gets matching acl host as *" do
    expect(build_report.acls[0].matching_acl_host).to eq("*")
  end

  it "gets matching acl operation as Rdkafka::Bindings::RD_KAFKA_ACL_OPERATION_READ" do
    expect(build_report.acls[0].matching_acl_operation).to eq(Rdkafka::Bindings::RD_KAFKA_ACL_OPERATION_READ)
  end

  it "gets matching acl permission_type as Rdkafka::Bindings::RD_KAFKA_ACL_PERMISSION_TYPE_ALLOW" do
    expect(build_report.acls[0].matching_acl_permission_type).to eq(Rdkafka::Bindings::RD_KAFKA_ACL_PERMISSION_TYPE_ALLOW)
  end
end
