  ...
  describe Rdkafka::Callbacks::BackgroundEventCallback do
    ...
    context "when an operation fails at the operation level (empty results, event error set)" do
      ...
      before do
        ...
        allow(Rdkafka::Bindings).to receive(:rd_kafka_event_type).with(event_ptr)
          .and_return(Rdkafka::Bindings::RD_KAFKA_EVENT_CREATETOPICS_RESULT)
        allow(Rdkafka::Bindings).to receive(:rd_kafka_event_CreateTopics_result).with(event_ptr).and_return(create_topics_result_ptr)
        # Empty results array - this is what makes results[0] nil
        allow(Rdkafka::Bindings).to receive(:rd_kafka_CreateTopics_result_topics) do |_result_ptr, count_ptr|
