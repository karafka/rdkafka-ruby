...
RSpec.describe Rdkafka::Consumer::Partition do
  let(:partition) { described_class.new(1, offset, err) }
  ...
  it "has an offset" do
    expect(partition.offset).to eq 100
  end
  ...
  describe "#to_s" do
    it "returns a human readable representation" do
      expect(partition.to_s).to eq "<Partition 1 offset=100>"
    end
  end
  ...
  describe "#inspect" do
    ...
    context "without offset" do
      ...
      it "returns a human readable representation" do
        expect(partition.to_s).to eq "<Partition 1>"
      end
    end
    ...
    context "with err code" do
      ...
      it "returns a human readable representation" do
        expect(partition.to_s).to eq "<Partition 1 offset=100 err=1>"
      end
    end
  end
  ...
  describe "#==" do
    it "equals another partition with the same content" do
      expect(partition).to eq described_class.new(1, 100)
    end

    it "does not equal another partition with different content" do
      expect(partition).not_to eq described_class.new(2, 101)
    end
  end
end
