...
## 0.29.1 (Unreleased)
- [Enhancement] Add transactional producer primitives: `Producer#init_transactions`, `#begin_transaction`, `#commit_transaction`, `#abort_transaction`, and `#send_offsets_to_transaction`, wrapping librdkafka's transactional API. `RdkafkaError.build_from_c` decodes the `rd_kafka_error_t*` these primitives return (and always destroys it) into a regular `RdkafkaError`, additionally exposing `#fatal?`, `#retryable?`, and `#abortable?` read directly off the pointer.
