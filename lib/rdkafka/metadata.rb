module Rdkafka
  class Metadata
    ...
    def initialize(native_client, topic_name = nil)
      ...
      # Error Handling
      RdkafkaError.new(result) unless result.zero?
    ...
    def metadata_from_native(ptr)
      ...
      @topics = Array.new(metadata[:topics_count]) do |i|
        ...
        RdkafkaError.new(topic[:rd_kafka_resp_err]) unless topic[:rd_kafka_resp_err].zero?
        ...
        partitions = Array.new(topic[:partition_count]) do |j|
          ...
          RdkafkaError.new(partition[:rd_kafka_resp_err]) unless partition[:rd_kafka_resp_err].zero?
          partition.to_h
        end
