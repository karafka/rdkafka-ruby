#!/usr/bin/env bash
#
# Build self-contained librdkafka.so for Linux aarch64 musl with checksum verification
# Usage: ./build_linux_aarch64_musl.sh
#
# Expected directory structure:
#   ext/build_linux_aarch64_musl.sh       (this script)
...
BUILD_DIR="$(pwd)/build-tmp-musl-aarch64"
DEPS_PREFIX="/tmp/musl-aarch64-deps"

# musl-specific dependency check
check_musl_dependencies() {
    log "Checking musl-specific build dependencies for ARM64..."
    ...
    if command -v ldd &> /dev/null && ldd --version 2>&1 | grep -q musl; then
        ...
        log "✅ musl detected via ARM64 loader presence"
    else
...
# Verify we're on ARM64 or in an appropriate environment
ARCH=$(uname -m)
if [[ "$ARCH" != "aarch64" && "$ARCH" != "arm64" ]]; then
    warn "Not running on aarch64 ($ARCH detected). This script is optimized for ARM64 systems."
    warn "Continuing anyway, but this may fail if not in an appropriate build environment..."
fi

# musl-specific compiler setup
setup_musl_compiler() {
    # musl-specific compiler flags for ARM64
    export CC="gcc"
    ...
    log "Applied musl-specific compiler flags for ARM64 (with C17 for GCC 15+ compatibility)"
}

# Build OpenSSL for musl on ARM64
build_openssl_musl() {
    ...
    if [ ! -f "$openssl_prefix/lib/libssl.a" ] && [ ! -f "$openssl_prefix/lib64/libssl.a" ]; then
        log "Configuring and building OpenSSL for musl on ARM64..."
        ...
        if [ -f "$openssl_prefix/lib/libssl.a" ] || [ -f "$openssl_prefix/lib64/libssl.a" ]; then
            ...
            log "✅ Static OpenSSL built successfully for ARM64"
        else
...
# Build MIT Kerberos for musl on ARM64
build_krb5_musl() {
    local krb5_prefix="$1"
    local krb5_dir="$2"
    ...
    if [ ! -f "$krb5_prefix/lib/libgssapi_krb5.a" ]; then
        log "Configuring and building MIT Kerberos for musl on ARM64..."
        make clean 2>/dev/null || true

        setup_musl_compiler

        # musl-specific configuration for Kerberos on ARM64
        ...
        log "✅ Static MIT Kerberos built successfully for ARM64"
    else
        log "Static MIT Kerberos already built, skipping..."
    fi
}

# Build Cyrus SASL for musl on ARM64 with proper OpenSSL linking
build_sasl_musl() {
    local sasl_prefix="$1"
    local sasl_dir="$2"
    local krb5_prefix="$3"
    ...
    if [ ! -f "$sasl_prefix/lib/libsasl2.a" ]; then
        log "Configuring and building Cyrus SASL for musl on ARM64..."
        make clean 2>/dev/null || true
        ...
        # Additional verification - check if OTP plugin was built
        ...
        log "✅ Static Cyrus SASL built successfully for ARM64"
    else
...
build_static_lib_musl() {
    ...
    if [ ! -f "$lib_file" ]; then
        log "Configuring and building static $lib_name for musl on ARM64..."
        make clean 2>/dev/null || true
        ...
        log "✅ Static $lib_name built successfully for ARM64"
    else
...
log "Building self-contained librdkafka.so for Linux aarch64 musl (WITH SASL)"
...
# Build OpenSSL
log "Building OpenSSL $OPENSSL_VERSION for musl on ARM64..."
...
# Build MIT Kerberos
log "Building MIT Kerberos $KRB5_VERSION for musl on ARM64..."
...
log "Building Cyrus SASL $CYRUS_SASL_VERSION for musl on ARM64..."
...
build_sasl_musl "$SASL_PREFIX" "$SASL_DIR" "$KRB5_PREFIX" "$OPENSSL_PREFIX"

cd "$BUILD_DIR"

# Build zlib
log "Building zlib $ZLIB_VERSION for musl on ARM64..."
...
# Build ZStd
log "Building ZStd $ZSTD_VERSION for musl on ARM64..."
...
if [ ! -f "$ZSTD_PREFIX/lib/libzstd.a" ]; then
    log "Building ZStd for musl on ARM64..."
    make clean 2>/dev/null || true
    ...
    log "✅ Static ZStd built successfully for ARM64"
else
...
# Configure librdkafka for musl on ARM64 (with SASL)
log "Configuring librdkafka for musl on ARM64 (with SASL support)..."
...
# Create self-contained shared library for musl on ARM64 (with SASL)
log "Creating self-contained librdkafka.so for musl on ARM64 (with SASL support)..."
...
log "Verifying musl build for ARM64..."
...
log "Verifying musl compatibility on ARM64..."
if command -v ldd &> /dev/null; then
...
# Enhanced summary for musl on ARM64 (with SASL)
log "Build completed successfully!"
log "📦 Self-contained librdkafka built for Linux aarch64 musl:"
...
log ""
log "🎯 Ready for deployment on ARM64 musl-based systems (Alpine Linux on ARM, Docker containers)"
...
log "🐳 Docker-optimized: Perfect for ARM Alpine-based container deployments"
log "🛠️  GCC 14+ compatible: Official patches applied to Cyrus SASL"
log "🚀 ARM64 native: Built for aarch64 architecture"
log ""
