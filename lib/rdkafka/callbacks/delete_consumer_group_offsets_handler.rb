...
module Rdkafka
  module Callbacks
    # Handles `RD_KAFKA_EVENT_DELETECONSUMERGROUPOFFSETS_RESULT` events
    # @private
    class DeleteConsumerGroupOffsetsHandler < BaseHandler
      class << self
        # Resolves the delete consumer group offsets handle from its result event
        # @param event_ptr [FFI::Pointer] pointer to the event
        # @return [void]
        def call(event_ptr)
          result = Rdkafka::Bindings.rd_kafka_event_DeleteConsumerGroupOffsets_result(event_ptr)

          pointer_to_size_t = FFI::MemoryPointer.new(:size_t)
          group_result_array = Rdkafka::Bindings.rd_kafka_DeleteConsumerGroupOffsets_result_groups(result, pointer_to_size_t)
          ...
          if (handle = Rdkafka::Admin::DeleteConsumerGroupOffsetsHandle.remove(handle_ptr.address))
            unless resolve_operation_error(event_ptr, handle)
              ...
              if group_result.nil?
                ...
                handle.result = Rdkafka::Admin::DeleteConsumerGroupOffsetsReport.new(
                  FFI::Pointer::NULL,
                  FFI::Pointer::NULL
                )
              else
                report = Rdkafka::Admin::DeleteConsumerGroupOffsetsReport.new(
