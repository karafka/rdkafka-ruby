...
module Rdkafka
  class Admin
    ...
    def create_acl(name, principal, host, acl_options = {})
      ...
      # Create a rd_kafka_AclBinding_new representing the new topic
      error_buffer = FFI::MemoryPointer.from_string(' ' * 256)
      new_acl_ptr = Rdkafka::Bindings.rd_kafka_AclBinding_new(
        resource_type,
        FFI::MemoryPointer.from_string(name),
        pattern_type, # pattern literal - used for any lookups
        FFI::MemoryPointer.from_string(principal),
        FFI::MemoryPointer.from_string(host),
        acl_operation, # allow any operation
        permission_type, # allow any permission type - permission type any
        error_buffer,
        512
      )
      ...
      if queue_ptr.null?
        Rdkafka::Bindings.rd_kafka_NewTopic_destroy(new_acl_ptr)
        raise Rdkafka::Config::ConfigError, 'rd_kafka_queue_get_background was NULL'
      end
    ...
    def create_topic(topic_name, partition_count, replication_factor, topic_config = {})
      ...
      raise Rdkafka::Config::ConfigError, error_buffer.read_string if new_topic_ptr.null?
    ...
    def closed_admin_check(method)
      raise Rdkafka::ClosedAdminError, method if closed?
    end
  end
end
